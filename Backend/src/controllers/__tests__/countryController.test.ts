...
describe('GET /api/countries', () => {
  ...
  it('should search for countries by name, capital, region, and subregion', async () => {
    // Search by name
    let response = await request(app).get('/api/countries/search?query=Country A').expect(200);
    expect(response.body.length).toBe(1);
    expect(response.body[0]).toHaveProperty('name', 'Country A');

    // Search by capital
    response = await request(app).get('/api/countries/search?query=Capital B').expect(200);
    expect(response.body.length).toBe(1);
    expect(response.body[0]).toHaveProperty('name', 'Country B');

    // Search by region
    response = await request(app).get('/api/countries/search?query=Region A').expect(200);
    expect(response.body.length).toBe(1);
    expect(response.body[0]).toHaveProperty('name', 'Country A');

    // Search by subregion
    response = await request(app).get('/api/countries/search?query=Subregion B').expect(200);
    expect(response.body.length).toBe(1);
    expect(response.body[0]).toHaveProperty('name', 'Country B');
  });
...
describe('GET /api/countries error scenarios', () => {
    it('should handle invalid search query', async () => {
      const response = await request(app).get('/api/countries/search?query=NonExistentCountry').expect(200);
    ...
    it('should handle unexpected errors in search', async () => {
      ...
      const response = await request(app).get('/api/countries/search?query=something').expect(500);
