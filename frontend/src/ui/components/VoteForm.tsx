...
export default function VoteForm() {
  ...
  const countriesData = useSelector((state: RootState) => state.countriesDataReducer.countriesData)
  const isLoading = useSelector((state: RootState) => state.submitVoteLoading.isLoading)
  ...
  return (
    ...
    >
      ...
      <Formik initialValues={{ name: '', email: '', countryCode: '' }} onSubmit={handleSubmit}>
            ...
            >
              ...
              >
                ...
                <Select
                  ...
                  onChange={(event) => setFieldValue('countryCode', event.target.value)}
                  label='Country'
                  required
                >
              ...
              <VoteButton disabled={!values.name || !values.email || !values.countryCode}>
                {isLoading ? <CircularProgress size={24} /> : constants.SUBMIT_VOTE_BUTTON_TITLE}
              </VoteButton>
            </Box>
          </form>
        )}
